        ...
        {REGION_LAYOUT.map((layout) => {
          ...
          return (
            <RegionTile
              ...
              onStartExam={() => onStartExamForRegion(stat.key, stat.name)}
            />
          );
        })}
      </div>
...
function RegionTile({ stat, onClick, onStartExam }: RegionTileProps) {
  const level =
  ...
  return (
    ...
    >
      <button className="region-tile-main" onClick={onClick}>
